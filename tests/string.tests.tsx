...
describe('string', () => {
    test('stringify', () => {
        ...
        const spec = m(
            'body',
            {class: {loaded: true, popup: false}, style: null},
            m(
                Array,
                null,
                m('input', {autofocus: true}, 'Content'),
                m(Component, null, m('b', null, 'Hello!')),
            ),
            null,
            [
                m(
                    'button',
                    {class: ['active', 'large'], onclick: () => alert('click')},
                    'Click Me',
                ),
                m(
                    'p',
                    {class: null, style: {color: 'red'}},
                    'Paragraph',
                    m('br', {oncreate: (node) => alert(node.outerHTML)}),
                    'Line 1\nLine 2',
                ),
                m('label', null, '<script>alert(`xss`);</script>'),
    ...
    test('plugins', () => {
        ...
        const html = stringify(
            m('div', null, m(Doc, null, ...specs), m('div', null, ...specs)),
        );
    ...
    test('unsupported spec', () => {
        expect(() =>
            stringify(m('div', null, document.createElement('a'))),
        ).toThrow(/Unable to stringify spec/);
        expect(() => stringify(m('div', null, (() => null) as any))).toThrow(
            /Unable to stringify spec/,
        );
        expect(() => stringify(m('div', null, true as any))).toThrow(
            /Unable to stringify spec/,
        );
        expect(() => stringify(m('div', null, 0 as any))).toThrow(
            /Unable to stringify spec/,
        );
        expect(() => stringify(m('div', null, {} as any))).toThrow(
            /Unable to stringify spec/,
        );
    ...
    test('escape', () => {
        expect(escapeHTML(`<script src='..' onload="&"></script>`)).toBe(

...
describe('state', () => {
    test('stateful component', () => {
        ...
        expect(element.innerHTML).toEqual(
            ['<div class="empty"><button>Apples</button> 0</div>'].join(''),
        );

        dispatchClick(element.querySelector('button'));
        expect(element.innerHTML).toEqual(
            ['<div><button>Apples</button> 1</div>'].join(''),
        );
        ...
        expect(element.innerHTML).toEqual(
            ['<div><button>Oranges</button> 1</div>'].join(''),
        );
        ...
        const Component = withState(({x}) => {
            ...
            return (
                <div>
                    <button
                        class="y"
                        onclick={() => setState({y: y + 1})}
                    ></button>
                    <button
                        class="z"
                        onclick={() => setState({z: z + 1})}
                    ></button>
                    <p>{`x: ${x}; y: ${y}; z: ${z};`}</p>
                </div>
            );
    ...
    test('prevent infinite recursion', () => {
        ...
        expect(() => render(container, <RecursiveComponent />)).toThrow(
            /infinite loop/,
        );
    ...
    test('call state after component update', () => {
        ...
        expect(element.outerHTML).toEqual(
            ['<div>', '2', '<button>', 'Checked', '</button>', '</div>'].join(
                '',
            ),
        );
    });

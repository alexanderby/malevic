...
describe('animation', () => {
    ...
    test('defaults', () => {
        ...
        const Label = withAnimation(() => (
            <label style={{opacity: animate().from(0).to(1)}} />
        ));
        const label = render(target, <Label />)
    ...
    test('string interpolation', () => {
        ...
        const box = target.firstElementChild as HTMLSpanElement;
        expect(box.getAttribute('transform')).toBe(
            'translate(0px) rotate(90deg)',
        );

        semaphore.tick(125);
        expect(box.getAttribute('transform')).toBe(
            'translate(50px) rotate(90deg)',
        );

        semaphore.tick(250);
        expect(box.getAttribute('transform')).toBe(
            'translate(100px) rotate(90deg)',
        );
    });
    ...
    test('interrupt animation', () => {
        const Box = withAnimation(({x, color}) => (
            <span
        ...
        render(target, <Box x={40} color={[255, 255, 255]} />);
        const box = target.firstElementChild as HTMLSpanElement;
        expect(box).toBeInstanceOf(HTMLSpanElement);
        expect(box.style.left).toBe('0px');
        ...
        render(target, <Box x={120} color={[0, 0, 0]} />);
    ...
    test('schedule and cancel attribute animation', () => {
        const Box = withAnimation(({x, isStatic}) => (
            <span
                style={
                    isStatic
                        ? `left: ${x}px;`
        ...
        render(target, <Box x={20} isStatic />);
        const box = target.firstElementChild as HTMLSpanElement;
        expect(box).toBeInstanceOf(HTMLSpanElement);
        expect(box.style.left).toBe('20px');
        ...
        render(target, <Box x={80} isStatic />);
        ...
        const Box = withAnimation(({x, isStatic}) => (
            <span
                style={{
                    ...
                    left: isStatic
                        ? `${x}px`
                        : animate(`${x}px`, {duration: 250, easing: 'linear'}),
                }}
            />
        ));
    ...
    test('animate another style property', () => {
        const Box = withAnimation(({left, right}) => (
            <span
                style={{
                    ...(left == null
                        ? {}
    ...
    test('easings', () => {
        const Box = withAnimation(({x, easing}) => (
            <span
    ...
    test('misuse', () => {
        expect(() => animate(30).from(40)).toThrow(
            /Starting keyframe was already declared/,
        );
    });
    ...
    test('stringify animation', () => {
        const Box = withAnimation(({x}) => (
            <Array>
                ...
                />
                <a class="second" style={animate(`bottom: ${x}px;`)} />
                <p
                    class="third"
                    style={{
                        right: 'initial',
    ...
    test('invalid value', () => {
        const C = withAnimation(() => (
            <div class={animate({}, {duration: 50}).initial({})} />
        ));
    ...
    test('animation type switch', () => {
        ...
        const child = (index: number) =>
            target.children[index] as HTMLDivElement;
